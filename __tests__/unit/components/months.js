...
import { monthOptions } from '../../../src/options/months';
...
it( 'change', () => {
    ...
    expect( changeDate.mock.calls[ 0 ][ 0 ].toISOString() ).toBe( '2019-04-23T06:02:17.000Z' );
    expect( meta.toJSON() ).toMatchSnapshot();
    ...
    expect( changeDate.mock.calls[ 2 ][ 0 ].toISOString() ).toBe( '2020-02-29T17:00:00.000Z' );
    expect( changeMode ).toHaveBeenCalledTimes( 1 );
    expect( changeMode.mock.calls[ 0 ][ 0 ] ).toBe( 'dom' );

    meta.ctx.container.querySelector( 'thead tr th:nth-child(2)' ).click();
    expect( changeMode ).toHaveBeenCalledTimes( 2 );
    expect( changeMode.mock.calls[ 1 ][ 0 ] ).toBe( 'year' );
} );
